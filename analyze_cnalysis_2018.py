...
# read in results
results = pd.read_csv(money_path + 'chaz\\chaz_prediction_results_2018.csv')
...
for conf in confidences:
    expected_win_margins[conf] = sts_df.loc['USA', conf + '_t_mean']
...
deg_f, _, sigma = sts.t.fit(no_safe['isolated_race_error'], floc=0)
st_deg_f, _, st_sigma = sts.t.fit(no_safe['state_R_overperformance'], floc=0)